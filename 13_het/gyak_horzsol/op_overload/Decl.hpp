...
class tort 
{
	...
	public: 			// a metódusok egyediek, minden objektum ezeket használja => *this
		...
		//operátor, visszatérési értéke tört lesz, nem igénylik az 1. tagot (t1)(bal érték) 2.tag(t2)(jobb érték) 3.tag(t3)(viszatérési érték) 
		...
		tort& operator*=(int i); 				// csak deklarálunk, nem definiálunk
		~tort() { }
		friend istream& operator>>(istream& be, tort& t);	  
};

tort operator*(tort e_t, tort m_t) {                  
    return tort(e_t.getSzaml() * m_t.getSzaml(), e_t.getNevz() * m_t.getNevz()); 		// this nem használható a külső függvényben
}
...
tort operator*(tort e_t, int i) {
    return tort(e_t.getSzaml() * i, e_t.getNevz()); 
}
...
tort operator*(int i, tort m_t) {
    return tort(i * m_t.getSzaml(), m_t.getNevz()); 
}
